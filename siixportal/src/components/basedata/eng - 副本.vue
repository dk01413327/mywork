<template>
    <div class="vue-component" >
        <div class="div-content">
            <button class="btn btn-primary"  @click="add" :disabled="getPermisson.permission.substr(0,1)==0">新增</button>
            
            <div class="div-table">
                <table class="table table-sm table-bordered table-hover mb-1 ">
                    <thead class="text-center"><tr ><th>工程名称</th><th>标准人力</th><th>实际人力</th><th>顺序</th><th>状态</th><th>备注</th>
                        <th>建立人</th><th>建立时间</th>
                        <th>更新人</th><th>更新时间</th><th>操作</th></tr></thead>
                    <tbody>
                        <tr v-for='(item,idx) in pagedata ? pagedata : engs' :key="idx" v-if="idx < pageSize">
                            <td>{{item.eng_name}}</td>
                            <td>{{item.standard_men}}</td>
                            <td>{{item.actual_men}}</td>
                            <td>{{item.seq}}</td>
                            <td class="text-center"><span class="text-white bg-danger p-1" v-if="item.enabled==0">禁用</span>
                                <span class="text-white bg-success p-1" v-else>启用</span> </td>                
                            <td>{{item.remark}}</td>
                            <td>{{item.creator}}</td>                        
                            ...
                            <td class="text-center">
                                <button class="btn btn-sm btn-success td-btn" @click="edit(idx)" :disabled="getPermisson.permission.substr(1,1)==0">修改</button>
                                <button class="btn btn-sm btn-success td-btn" @click="editman(idx)" :disabled="getPermisson.permission.substr(1,1)==0">人力</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="modal-body">
                    <form role="form" id="form-man" >                       
                        ...
                        <div class="form-group row" v-for="(item,index) in 4" :key="index">
                            ...
                            <div class="col-3">
                                <input class="form-control man" name="man[]" :id="'man'+(item*3-2)" :placeholder="(item*3-2)+'月'">
                                <input class="form-control man" name="idl[]" :id="'idl'+(item*3-2)" :placeholder="(item*3-2)+'月'">
                            </div>
                            <div class="col-3">
                                <input class="form-control man" name="man[]" :id="'man'+(item*3-1)" :placeholder="(item*3-1)+'月'">
                                <input class="form-control man" name="idl[]" :id="'idl'+(item*3-1)" :placeholder="(item*3-1)+'月'">
                            </div>
                            <div class="col-3">
                                <input class="form-control man" name="man[]" :id="'man'+(item*3)" :placeholder="(item*3)+'月'">
...
export default {
    ...
    computed:{
        ...mapGetters(['getPermisson','userinfo'])
    },
    created(){
        ...
        getengman(){
            ...
            this.$store.dispatch('getEngMan',{'data':data,'cb':(res)=>{
                ...
                res.data.forEach((element,index) => {
                    $('#man'+(index+1)).val(element.actual_men)
                });
            }})
        }
    }
